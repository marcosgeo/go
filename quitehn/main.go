...
func main() {
	...
	tpl := template.Must(template.ParseFiles("./quitehn/index.gohtml"))
...
func getCachedStories(numStories int) ([]item, error) {
	...
	cache = stories
	cacheExpiration = time.Now().Add(2 * time.Second)
	return stories, nil
}
...
func getStories(ids []int) []item {
	...
	for i := 0; i < len(ids); i++ {
		go func(idx, id int) {
			var client hn.Client // since hn.Client is a small struct, no significant impact
...
// item is the same as the hn.Item, but add the Host field
type item struct {
	hn.Item
	Host string
}
